...
2.  **Environment Variables:**
    *   Copy the `.env.sample` file to a new file named `.env` (if it doesn't exist).
    *   Update the `.env` file with your Google Cloud project details. These are **required** for the agent to connect to the Vertex AI endpoint:
        ```bash
        # .env
    ...
    *   Replace `YOUR_REGION` with the desired Google Cloud region where the Vertex AI endpoint is available. Choose a region geographically close to you for lower latency. You can find a list of available regions for Vertex AI Generative AI [here](https://cloud.google.com/vertex-ai/generative-ai/docs/learn/locations#available-regions). For example, if you are in Arizona, `us-west4` (Las Vegas) might be a good choice.
    *   The project now uses the `litellm` library to interact with the Vertex AI endpoint. This dependency is managed automatically by `uv` based on the `pyproject.toml` file.

3.  **Google Cloud Authentication:**
    *   You need to authenticate with Google Cloud so the agent can obtain an access token to use the Vertex AI API. Run the following command in the Dev Container terminal and follow the prompts to log in:
        ```bash
